  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      describe 'Testing the dependencies between the classes' do
        ...
        it { is_expected.to contain_class('bht::dependencies').that_comes_before('Class[bht::config]') }
        it { is_expected.to contain_class('bht::config').that_comes_before('Class[bht::install]') }
      end
    end
  end
end
